...
import { tagOptions } from '../../../components/notes/NoteModal';
import { useTranslation } from 'react-i18next';
import './NotesApp.css';

// Main wrapper component for the Notes dashboard, orchestrating the Sidebar, NotesHeader, NoteCards, and Modal
const NotesApp = ({ cardStyle }) => {
    ...
    // Derived state: Apply search, filters (category/status), and sorting to the raw notes array
    ...
    // Determines the appropriate placeholder message and icon when no notes match the current filters
    ...
    // Reset workspace state (filters, selection, sorts) when changing primary views (e.g., active -> archive)
    useEffect(() => {
    ...
    // Intercepts Ctrl+A / Cmd+A to select all visible notes instead of standard text selection (unless in an input)
    useEffect(() => {
    ...
    // Global keyboard shortcut handler for actions like Save, New Note, Search, Escape, etc.
    useEffect(() => {
    ...
    // Handles individual note selection, supporting Shift-Click for multi-range selection
    ...
    // Auto-deselects notes if they are filtered out of the current view (e.g., archived while selected)
    useEffect(() => {
    ...
    // Groups notes by date ranges (Today, 7 Days, 30 Days, Month/Year) if sorting by date is active

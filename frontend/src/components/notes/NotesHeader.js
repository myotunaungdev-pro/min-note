...
// Top action bar for the notes dashboard containing search, sorting, filtering, and bulk actions
const NotesHeader = ({ onSelectAll }) => {
    ...
    // Adds a visual drop shadow to the header when the user scrolls down the page
    useEffect(() => {
    ...
    // Handles the delete action based on current view: Soft delete (Trash) or Permanent delete
    ...
    // Global keyboard shortcut (Ctrl+K or Cmd+K) to focus the search bar
    useEffect(() => {
    ...
    // Auto-focuses the mobile search input when the search modal is opened
    useEffect(() => {
    ...
    // Permanently removes the selected notes from the database and clears the selection state

...
// Formats HTML spacing to render correctly outside the editor (converts spaces/tabs to non-breaking entities)
...
// Generates a cropped version of an image based on crop coordinates using an HTML5 Canvas
...
// Resizes and compresses images heavily to save on Cloudinary storage before uploading
...
// Uploads compressed images directly to Cloudinary using an unsigned upload preset
...
// A sub-modal allowing users to crop an image they've added to a note
...
// A sub-modal providing a drawing canvas over an image for annotating or sketching
...
// A sub-modal that accesses the device camera to take photos directly into the note editor
...
// Action menu shown when clicking the image button in the editor (Camera, Gallery, OCR)
...
// Read-only modal displaying formatted note contents and an image gallery viewer
...
// Editor modal containing the React-Quill rich text editor, theme pickers, and tags
const NoteEditModal = () => {
    const { t } = useTranslation();
    const dispatch = useDispatch();
...
// Wrapper component that mounts either the Read-Only viewer or the Editor Modal based on state
const NoteModal = () => {
    const dispatch = useDispatch();

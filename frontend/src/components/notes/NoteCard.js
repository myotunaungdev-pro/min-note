...
// Renders an individual note card supporting various styles (Default, Cyber, Dynamic 3D) and actions (Edit, Delete, Archive, Done)
const NoteCard = ({ note, onDeleteRequest, onSelectToggle, cardStyle = 'default' }) => {
    ...
    // Calculates the 3D tilt effect based on mouse position relative to the center of the card
    ...
    // Opens the Note Reader overlay, avoiding conflict if the user clicked a link inside the note
    ...
    // Dispatches the note to the Redux store to be loaded into the Edit Modal
    const handleEdit = (e) => {
        e.stopPropagation();
    ...
    // Toggles the archived state and triggers a server update
    ...
    // Moves the note to the trash by toggling isDeleted
    ...
    // Restores a note from the trash back to its previous state (All Notes)
    ...
    // Toggles the completion status (Done/Pending) of the note task
    ...
    // Triggers the permanent deletion confirmation callback for notes in the trash

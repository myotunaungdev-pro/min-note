...
// Utility to map backend error messages to translation keys for internationalization
...
// Dispatches a POST request to register a new user and returns the success message
...
// Verifies the code sent to the user's email; on success, it stores the JWT and logs them in
...
// Submits the reset OTP alongside the new password to regain account access
...
// Authenticates a returning user and caches their session in localStorage
...
// Updates demographic info or avatar, optimistically updating the UI state
...
// Called on initial app load to hydrate Redux state with fresh data from the DB using the stored token
...
// Async thunk to permanently dismiss the Pro welcome modal after the user closes it
...
// Helper function to persist critical user state across browser refreshes
...
// Helper function to completely wipe local storage on logout
...
// Safely parses the cached user object during the initial Redux state hydration
...
// Redux slice defining synchronous actions and reducers for authentication
